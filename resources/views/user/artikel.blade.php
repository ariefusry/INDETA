...
<body class="bg-[#f0eadd] text-gray-800 font-sans antialiased overflow-x-hidden">
    ...
    <!-- Main Section -->
    <section class="relative min-h-[90vh] py-12 bg-cover bg-center bg-no-repeat" style="background-image: url('https://images.unsplash.com/photo-1506744626753-1fa44df31c7f?q=80&w=2070&auto=format&fit=crop');">
        <!-- Dark Overlay -->
        ...
        <div class="relative z-10 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between mb-12 relative w-full pt-4">
                <div class="w-full flex justify-center md:absolute md:inset-0 md:pointer-events-none items-center mt-2 md:mt-0">
                    <h1 class="text-3xl md:text-5xl font-bold text-white tracking-wide shadow-black drop-shadow-md">Artikel</h1>
                </div>
    ...
    <script>
      ...
      function renderArticles(destinationsToRender) {
        if (destinationsToRender.length === 0) {
            destinationsGrid.innerHTML = '<div class="w-full text-center text-white py-10 text-xl drop-shadow-md">Tidak ada data artikel ditemukan.</div>';
            return;
          }
          ...
          destinationsGrid.innerHTML = destinationsToRender.map(dest => {
            ...
            return `
              <a href="/artikel/${dest.slug}" class="w-[calc(100%-0.5rem)] sm:w-[calc(50%-0.5rem)] md:w-[calc(33.333%-1rem)] relative aspect-[3/4] bg-gray-200 rounded-3xl overflow-hidden shadow-xl group cursor-pointer hover:-translate-y-2 transition-all duration-300 block">
                  ...
                  <div class="absolute bottom-0 left-0 w-full p-5 md:p-6 text-left">
                      <h3 class="text-white font-bold text-xl md:text-2xl drop-shadow-md leading-tight">${dest.title}</h3>
                  </div>
              </a>
            `;
        }).join('');
